...
import ComicFormDialog, { Comic } from "@/components/groups/dialog/ComicFormDialog";
import {
  AlertDialog,
  AlertDialogAction,
  AlertDialogCancel,
...
export default function ManageComics() {
  ...
  const fetchComics = async (page = 1) => {
    try {
      ...
      const res = await axios.get<ApiComicsResponse>(
        `${API_URL}/translator/${groupId}/comics`,
  ...
  const handleConfirmDelete = async () => {
    ...
    setDeleting(true);
    try {
      await axios.delete(`${API_URL}/translator/comics/${deleteTarget.id}`, {
        headers: { ...authHeader() },
      });
      // Cập nhật list local
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground">Quản lý tất cả truyện tranh của nhóm</p>
        </div>
        <ComicFormDialog
          mode="add"
