...
export default function ProfilePage() {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        ...
        const goldData = resGold.data.data;
        setTransactionHistory(goldData.transactionHistory);
        setDailyCheckin(goldData.dailyCheckin);
        setDayStreak(goldData.dayStreak);
        setLongestStreak(goldData.longestStreak);
  ...
  const handleCheckin = (
    updatedCheckin: DailyCheckinItem[],
    newGold: number
  ) => {
    setDailyCheckin(updatedCheckin);
    setUser((prev) =>
      prev ? { ...prev, goldCoins: newGold } : prev
    );
  };
  ...
  const handleClaimQuest = async (questId: number) => {
    try {
      ...
      if (response.data.success && response.data.data) {
        setUser((prev) =>
          prev
            ? { ...prev, goldCoins: response.data.data!.newBalance }
            : prev
        );
      ...
      <div className="container mx-auto px-4 py-8">
        <div className="flex flex-col lg:flex-row gap-6">
            ...
            <Card className="bg-card/50 backdrop-blur-sm border-border/50">
              <CardContent className="p-6">
                  ...
                  <div className="text-center w-full">
                    ...
                    <div className="w-full mt-2">
                      <div className="flex justify-between text-xs text-muted-foreground mb-1">
                        ...
                        <span>
                          {user.experience.current} / {user.experience.max} EXP
                        </span>
                      </div>
                ...
                <nav className="hidden lg:block mt-6 space-y-2">
                  ...
                  <Button
                    variant={activeTab === "gold" ? "default" : "ghost"}
                    className="w-full justify-start"
          ...
          <div className="lg:w-3/4">
            {activeTab === "info" && (
              <ProfileInfoTab user={user} onUserChange={handleUserUpdated} />
            )}
